...
func TestEvaluate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			indicies: selector.Indices{CommitIndexes: mapset.NewSet[int](1, 2, 3)},
		},
		{
			desc:     "range",
			input:    "1-3",
		...
		},
		{
			...
			indicies: selector.Indices{CommitIndexes: mapset.NewSet[int](1, 2, 3, 4, 5, 6, 8)},
		},
		{
			desc:     "with destination",
			input:    "s0:1-3",
			commits:  testingCommits(9, CommitToPr{}),
			indicies: selector.Indices{DestinationPRIndex: ptr(0), CommitIndexes: mapset.NewSet[int](1, 2, 3)},
		},
		{
			desc:     "with empty selector",
			input:    "s0:",
			indicies: selector.Indices{DestinationPRIndex: ptr(0), CommitIndexes: mapset.NewSet[int]()},
		},
		{
			desc:     "with additive destination",
			input:    "s0+5-7",
			commits:  testingCommits(9, CommitToPr{1: 0, 2: 0, 3: 0}),
			indicies: selector.Indices{DestinationPRIndex: ptr(0), CommitIndexes: mapset.NewSet[int](1, 2, 3, 5, 6, 7)},
		}, {
			desc:     "with duplicates",
			input:    "s0+5-7,4-6,8,9",
			commits:  testingCommits(10, CommitToPr{1: 0, 2: 0, 3: 0}),
			indicies: selector.Indices{DestinationPRIndex: ptr(0), CommitIndexes: mapset.NewSet[int](1, 2, 3, 4, 5, 6, 7, 8, 9)},
		},
		{
